...
djd43.provide("djd43.widget.TimePicker");
...
djd43.requireLocalization("djd43.widget", "TimePicker", null, "ROOT");
djd43.widget.defineWidget("djd43.widget.TimePicker", djd43.widget.HtmlWidget, function () {
	this.time = "";
	...
	if (this.storedTime.indexOf("T") != -1 && this.storedTime.split("T")[1] && this.storedTime != " " && this.storedTime.split("T")[1] != "any") {
		this.time = djd43.widget.TimePicker.util.fromRfcDateTime(this.storedTime, this.useDefaultMinutes, this.selectedTime.anyTime);
	} else {
		if (this.useDefaultTime) {
			this.time = djd43.widget.TimePicker.util.fromRfcDateTime("", this.useDefaultMinutes, this.selectedTime.anyTime);
		} else {
			this.selectedTime.anyTime = true;
			this.time = djd43.widget.TimePicker.util.fromRfcDateTime("", 0, 1);
		}
	}
}, initUI:function () {
	if (!this.selectedTime.anyTime && this.time) {
		var amPmHour = djd43.widget.TimePicker.util.toAmPmHour(this.time.getHours());
		var hour = amPmHour[0];
		var isAm = amPmHour[1];
	...
	if (date) {
		...
		this.setDateTime(djd43.date.toRfc3339(date));
	} else {
		this.selectedTime.anyTime = true;
	}
	this.initData();
	...
	if (evt && evt.target) {
		if (evt.target.nodeType == djd43.dom.ELEMENT_NODE) {
			var eventTarget = evt.target;
		} else {
			var eventTarget = evt.target.parentNode;
		}
		djd43.event.browser.stopEvent(evt);
		djd43.html.setClass(eventTarget, this.classNames.selectedTime);
		this.selectedTime["hour"] = eventTarget.innerHTML;
	} else {
		if (!isNaN(evt)) {
			var hourNodes = this.hourContainerNode.getElementsByTagName("td");
			if (hourNodes.item(evt)) {
				djd43.html.setClass(hourNodes.item(evt), this.classNames.selectedTime);
	...
	if (evt && evt.target) {
		...
		if (!isNaN(evt)) {
			...
			if (minuteNodes.item(evt)) {
				djd43.html.setClass(minuteNodes.item(evt), this.classNames.selectedTime);
	...
	var eventTarget = evt.target;
	if (evt && eventTarget) {
		if (eventTarget.nodeType != djd43.dom.ELEMENT_NODE) {
		...
		if (amPmNodes.item(evt)) {
			...
			djd43.html.setClass(amPmNodes.item(evt), this.classNames.selectedTime);
		}
	}
	...
	this.selectedTime.anyTime = true;
	djd43.html.setClass(this.anyTimeContainerNode, this.classNames.selectedTime + " " + "anyTimeContainer");
}, onClick:function (evt) {
	djd43.event.browser.stopEvent(evt);
}, onSetTime:function () {
	if (this.selectedTime.anyTime) {
		this.time = new Date();
		var tempDateTime = djd43.widget.TimePicker.util.toRfcDateTime(this.time);
		...
		this.time = new Date();
		this.time.setHours(djd43.widget.TimePicker.util.fromAmPmHour(hour, isAm));
		this.time.setMinutes(minute);
		this.setDateTime(djd43.widget.TimePicker.util.toRfcDateTime(this.time));
	}
	this.onValueChanged(this.time);
}, onValueChanged:function (date) {
}});
djd43.widget.TimePicker.util = new function () {
	this.toRfcDateTime = function (jsDate) {
		if (!jsDate) {
			jsDate = new Date();
		}
		jsDate.setSeconds(0);
		return djd43.date.strftime(jsDate, "%Y-%m-%dT%H:%M:00%z");
	};
