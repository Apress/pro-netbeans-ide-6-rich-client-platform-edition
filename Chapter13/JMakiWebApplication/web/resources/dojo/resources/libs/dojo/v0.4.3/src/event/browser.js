...
djd43.provide("djd43.event.browser");
djd43.require("djd43.event.common");
djd43._ie_clobber = new function () {
	this.clobberNodes = [];
...
if (djd43.render.html.ie) {
	djd43.addOnUnload(function () {
		djd43._ie_clobber.clobber();
		try {
			if ((djd43["widget"]) && (djd43.widget["manager"])) {
				djd43.widget.manager.destroyAll();
			}
		}
		catch (e) {
		}
		if (djd43.widget) {
			for (var name in djd43.widget._templateCache) {
				if (djd43.widget._templateCache[name].node) {
					djd43.dom.destroyNode(djd43.widget._templateCache[name].node);
					djd43.widget._templateCache[name].node = null;
					delete djd43.widget._templateCache[name].node;
				}
			}
		}
		try {
		...
		djd43._ie_clobber.clobberNodes = [];
	});
}
djd43.event.browser = new function () {
	var clobberIdx = 0;
	...
	this.clean = function (node) {
		if (djd43.render.html.ie) {
			djd43._ie_clobber.clobber(node);
		}
	};
	this.addClobberNode = function (node) {
		...
		if (!node["__doClobber__"]) {
			...
			djd43._ie_clobber.clobberNodes.push(node);
	...
	this.removeListener = function (node, evtName, fp, capture) {
		...
		evtName = djd43.event.browser.normalizedEventName(evtName);
		if (evtName == "key") {
			if (djd43.render.html.ie) {
	...
	this.addListener = function (node, evtName, fp, capture, dontFix) {
		...
		if (!dontFix) {
			var newfp = function (evt) {
				...
				var ret = fp(djd43.event.browser.fixEvent(evt, this));
				if (capture) {
	...
	this.callListener = function (listener, curTarget) {
		if (typeof listener != "function") {
			djd43.raise("listener not a function: " + listener);
		}
		djd43.event.browser.currentEvent.currentTarget = curTarget;
		return listener.call(curTarget, djd43.event.browser.currentEvent);
	};
	this._stopPropagation = function () {
		djd43.event.browser.currentEvent.cancelBubble = true;
	};
	this._preventDefault = function () {
		djd43.event.browser.currentEvent.returnValue = false;
	};
	...
	this.fixEvent = function (evt, sender) {
		...
		if ((evt["type"]) && (evt["type"].indexOf("key") == 0)) {
			...
			if (evt["type"] == "keydown" && djd43.render.html.ie) {
				switch (evt.keyCode) {
				  case evt.KEY_SHIFT:
				  case evt.KEY_CTRL:
				  case evt.KEY_ALT:
		...
		if (djd43.render.html.ie) {
			...
			var docBody = ((djd43.render.html.ie55) || (doc["compatMode"] == "BackCompat")) ? doc.body : doc.documentElement;
			if (!evt.pageX) {
