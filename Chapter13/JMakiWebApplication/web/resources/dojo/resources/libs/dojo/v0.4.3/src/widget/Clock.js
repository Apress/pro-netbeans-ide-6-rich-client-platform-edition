...
djd43.provide("djd43.widget.Clock");
djd43.require("djd43.widget.*");
djd43.require("djd43.gfx.*");
djd43.require("djd43.uri.Uri");
djd43.require("djd43.lang.common");
djd43.require("djd43.lang.timing.Timer");
djd43.widget.defineWidget("djd43.widget.Clock", djd43.widget.HtmlWidget, function () {
	var self = this;
        this.showAMPM = true;
	...
	this.timer = new djd43.lang.timing.Timer(1000);
	...
	this.image = djd43.uri.moduleUri("djd43.widget", "templates/images/clock.png");
	this.surface = null;
	this.labelNode = null;
	...
	this.draw = function () {
		...
		self.placeSecond(s);
		if (this.showAMPM == true)self.topLabelNode.innerHTML = ((self.date.getHours() + self.timeZoneOffset) > 11) ? "PM" : "AM";
	};
	...
	var first = true;
	djd43.lang.forEach(points, function (c) {
		if (first) {
			path.moveTo(c.x, c.y);
			first = false;
		} else {
	...
	return shape.setTransform([move, djd43.gfx.matrix.rotateg(-angle)]);
	...
	this.surface = djd43.gfx.createSurface(this.domNode, 150, 150);
	...
	if (this.showAMPM == true)this.topLabelNode.innerHTML = ((this.date.getHours() + this.timeZoneOffset) > 11) ? "PM" : "AM";
