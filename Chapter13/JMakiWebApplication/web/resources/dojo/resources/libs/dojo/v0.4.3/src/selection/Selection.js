...
djd43.provide("djd43.selection.Selection");
djd43.require("djd43.lang.array");
djd43.require("djd43.lang.func");
djd43.require("djd43.lang.common");
djd43.require("djd43.math");
djd43.declare("djd43.selection.Selection", null, {initializer:function (items, isCollection) {
	this.items = [];
	this.selection = [];
	this._pivotItems = [];
	this.clearItems();
	...
	if (inSelection) {
		return djd43.lang.find(this.selection, item);
	} else {
		return djd43.lang.find(this.items, item);
	}
	...
	var args = djd43.lang.unnest(arguments);
	for (var i = 0; i < args.length; i++) {
		this.items.push(args[i]);
	}
}, addItemsAt:function (item, before) {
	if (this.items.length == 0) {
		return this.addItems(djd43.lang.toArray(arguments, 2));
	}
	if (!this.isItem(item)) {
	...
	if (this.lastSelected) {
		...
		var range = djd43.math.range(lastIdx, fromIdx, step);
	...
	var range = djd43.math.range(toIdx, fromIdx, step);
	if (range.length) {
	...
	var i = djd43.lang.find(this._pivotItems, item);
	if (i > -1) {
	...
	return djd43.lang.toArray(this.selection).sort(djd43.lang.hitch(this, function (a, b) {
