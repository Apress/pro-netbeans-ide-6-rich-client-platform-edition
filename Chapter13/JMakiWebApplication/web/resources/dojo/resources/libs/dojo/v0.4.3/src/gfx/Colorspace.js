...
djd43.provide("djd43.gfx.Colorspace");
djd43.require("djd43.lang.common");
djd43.require("djd43.math.matrix");
djd43.gfx.Colorspace = function () {
	this.whitePoint = "D65";
	this.stdObserver = "10";
...
djd43.gfx.Colorspace.prototype.convert = function (col, model_from, model_to) {
	...
	if (this[k]) {
		...
		if (this.chainMaps[k]) {
			...
			djd43.debug("Can't convert from " + model_from + " to " + model_to);
		}
	}
};
djd43.gfx.Colorspace.prototype.munge = function (keys, args) {
	if (djd43.lang.isArray(args[0])) {
		args = args[0];
	}
	var out = new Array();
...
djd43.gfx.Colorspace.prototype.getWhitePoint = function () {
	var x = 0;
	var y = 0;
	var t = 0;
	  ...
	  case "2":
		switch (this.whitePoint) {
		  ...
		  case "F11":
			...
			break;
		  default:
			djd43.debug("White point " + this.whitePoint + " isn't defined for Std. Observer " + this.strObserver);
		}
		break;
	  case "10":
		switch (this.whitePoint) {
		...
		break;
	  default:
		djd43.debug("Std. Observer " + this.strObserver + " isn't defined");
	}
	var z = 1 - x - y;
...
djd43.gfx.Colorspace.prototype.getPrimaries = function () {
	var m = [];
	switch (this.RGBWorkingSpace) {
	  ...
	  case "wide_gamut_rgb":
		...
		break;
	  default:
		djd43.debug("RGB working space " + this.RGBWorkingSpace + " isn't defined");
	}
...
djd43.gfx.Colorspace.prototype.epsilon = function () {
	return this.useApproxCIELabMapping ? 0.008856 : 216 / 24289;
};
djd43.gfx.Colorspace.prototype.kappa = function () {
	return this.useApproxCIELabMapping ? 903.3 : 24389 / 27;
};
djd43.gfx.Colorspace.prototype.XYZ_to_xyY = function () {
...
djd43.gfx.Colorspace.prototype.xyY_to_XYZ = function () {
...
djd43.gfx.Colorspace.prototype.RGB_to_XYZ = function () {
	...
	var XYZ = djd43.math.matrix.multiply([[r, g, b]], m);
	return [XYZ[0][0], XYZ[0][1], XYZ[0][2]];
};
djd43.gfx.Colorspace.prototype.XYZ_to_RGB = function () {
	var src = this.munge("XYZ", arguments);
	var mi = this.getXYZ_RGB_Matrix();
	var pr = this.getPrimaries();
	var rgb = djd43.math.matrix.multiply([[src.X, src.Y, src.Z]], mi);
	var r = rgb[0][0];
	var g = rgb[0][1];
	var b = rgb[0][2];
	if (this.RGBWorkingSpace == "s_rgb") {
...
djd43.gfx.Colorspace.prototype.XYZ_to_Lab = function () {
	var src = this.munge("XYZ", arguments);
	var wp = this.getWhitePoint();
	var xr = src.X / wp.X;
	var yr = src.Y / wp.Y;
...
djd43.gfx.Colorspace.prototype.Lab_to_XYZ = function () {
...
djd43.gfx.Colorspace.prototype.Lab_to_LCHab = function () {
	var src = this.munge("Lab", arguments);
	var L = src.L;
...
djd43.gfx.Colorspace.prototype.LCHab_to_Lab = function () {
...
djd43.gfx.Colorspace.prototype.chromaticAdaptation = function (col, src_w, dst_w) {
	...
	switch (this.chromaticAdaptationAlg) {
	  ...
	  case "von_kries":
		...
		break;
	  default:
		djd43.debug("The " + this.chromaticAdaptationAlg + " chromatic adaptation algorithm matricies are not defined");
	}
	var domain_src = djd43.math.matrix.multiply([[wp_src.x, wp_src.y, wp_src.z]], ma);
	var domain_dst = djd43.math.matrix.multiply([[wp_dst.x, wp_dst.y, wp_dst.z]], ma);
	var centre = [[domain_dst[0][0] / domain_src[0][0], 0, 0], [0, domain_dst[0][1] / domain_src[0][1], 0], [0, 0, domain_dst[0][2] / domain_src[0][2]]];
	var m = djd43.math.matrix.multiply(djd43.math.matrix.multiply(ma, centre), mai);
	var dst = djd43.math.matrix.multiply([[col.X, col.Y, col.Z]], m);
	return dst[0];
};
djd43.gfx.Colorspace.prototype.getRGB_XYZ_Matrix = function () {
	var wp = this.getWhitePoint();
	var pr = this.getPrimaries();
	var Xr = pr.xr / pr.yr;
	var Yr = 1;
	...
	var sm = djd43.math.matrix.multiply(m2, djd43.math.matrix.inverse(m1));
	var Sr = sm[0][0];
	var Sg = sm[0][1];
	var Sb = sm[0][2];
...
djd43.gfx.Colorspace.prototype.getXYZ_RGB_Matrix = function () {
	var m = this.getRGB_XYZ_Matrix();
	return djd43.math.matrix.inverse(m);
};
djd43.gfx.Colorspace.prototype.XYZ_to_Luv = function () {
	var src = this.munge("XYZ", arguments);
	var wp = this.getWhitePoint();
...
djd43.gfx.Colorspace.prototype.Luv_to_XYZ = function () {
...
djd43.gfx.Colorspace.prototype.Luv_to_LCHuv = function () {
	var src = this.munge("Luv", arguments);
	var L = src.L;
...
djd43.gfx.Colorspace.prototype.LCHuv_to_Luv = function () {
	var src = this.munge("LCH", arguments);
	var H_rad = src.H * (Math.PI / 180);
	var L = src.L;
...
djd43.gfx.Colorspace.colorTemp_to_whitePoint = function (T) {
	if (T < 4000) {
		djd43.debug("Can't find a white point for temperatures under 4000K");
		return [0, 0];
	}
	if (T > 25000) {
		djd43.debug("Can't find a white point for temperatures over 25000K");
		return [0, 0];
	}
	var T1 = T;
	var T2 = T * T;
...
djd43.gfx.Colorspace.prototype.RGB_to_CMY = function () {
	var src = this.munge("RGB", arguments);
	var C = 1 - src.R;
	var M = 1 - src.G;
	var Y = 1 - src.B;
...
djd43.gfx.Colorspace.prototype.CMY_to_RGB = function () {
...
djd43.gfx.Colorspace.prototype.RGB_to_CMYK = function () {
	var src = this.munge("RGB", arguments);
...
djd43.gfx.Colorspace.prototype.CMYK_to_RGB = function () {
...
djd43.gfx.Colorspace.prototype.CMY_to_CMYK = function () {
	var src = this.munge("CMY", arguments);
...
djd43.gfx.Colorspace.prototype.CMYK_to_CMY = function () {
	var src = this.munge("CMYK", arguments);
...
djd43.gfx.Colorspace.prototype.RGB_to_HSV = function () {
	var src = this.munge("RGB", arguments);
...
djd43.gfx.Colorspace.prototype.HSV_to_RGB = function () {
...
djd43.gfx.Colorspace.prototype.RGB_to_HSL = function () {
	var src = this.munge("RGB", arguments);
	var min = Math.min(src.R, src.G, src.B);
...
djd43.gfx.Colorspace.prototype.HSL_to_RGB = function () {
