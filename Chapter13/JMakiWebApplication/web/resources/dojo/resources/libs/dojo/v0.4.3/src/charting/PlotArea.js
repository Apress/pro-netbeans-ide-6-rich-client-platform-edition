...
djd43.provide("djd43.charting.PlotArea");
djd43.require("djd43.lang.common");
djd43.require("djd43.gfx.color");
djd43.require("djd43.gfx.color.hsl");
djd43.require("djd43.charting.Plot");
djd43.charting.PlotArea = function () {
	var id = "dojo-charting-plotarea-" + djd43.charting.PlotArea.count++;
	this.getId = function () {
		return id;
	};
...
djd43.charting.PlotArea.count = 0;
djd43.extend(djd43.charting.PlotArea, {nextColor:function () {
	var rgb = djd43.gfx.color.hsl2rgb(this._color.h, this._color.s, this._color.l);
	this._color.h = (this._color.h + this._color.step) % 360;
	while (this._color.h < 140) {
		this._color.h += this._color.step;
	}
	return djd43.gfx.color.rgb2hex(rgb[0], rgb[1], rgb[2]);
}, getArea:function () {
	...
	for (var i = 0; i < this.plots.length; i++) {
		...
		switch (plot.renderType) {
		  case djd43.charting.RenderPlotSeries.Grouped:
			if (plot.series[0]) {
				target.appendChild(plot.series[0].plotter(this, plot, kwArgs, applyToData));
			}
			break;
		  case djd43.charting.RenderPlotSeries.Singly:
		  default:
...
djd43.requireIf(djd43.render.svg.capable, "djd43.charting.svg.PlotArea");
djd43.requireIf(djd43.render.vml.capable, "djd43.charting.vml.PlotArea");
