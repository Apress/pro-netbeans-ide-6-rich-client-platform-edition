...
djd43.provide("djd43.charting.svg.Plotters");
djd43.require("djd43.lang.common");
if (djd43.render.svg.capable) {
	djd43.require("djd43.svg");
	djd43.mixin(djd43.charting.Plotters, {Bar:function (plotarea, plot, kwArgs, applyTo) {
		var area = plotarea.getArea();
		var group = document.createElementNS(djd43.svg.xmlns.svg, "g");
		...
		for (var i = n - 1; i >= 0; i--) {
			var path = document.createElementNS(djd43.svg.xmlns.svg, "path");
			...
			for (var j = 0; j < data[i].length; j++) {
				...
				cmd.push(x + "," + y);
				var c = document.createElementNS(djd43.svg.xmlns.svg, "circle");
				c.setAttribute("cx", x);
				c.setAttribute("cy", y);
				c.setAttribute("r", "3");
		...
		var area = plotarea.getArea();
		var line = document.createElementNS(djd43.svg.xmlns.svg, "g");
		if (data.length == 0) {
			return line;
		}
		var path = document.createElementNS(djd43.svg.xmlns.svg, "path");
		...
		for (var i = 0; i < n; i++) {
			...
			var g = document.createElementNS(djd43.svg.xmlns.svg, "g");
			var bar = document.createElementNS(djd43.svg.xmlns.svg, "rect");
			bar.setAttribute("fill", data[i].series.color);
			bar.setAttribute("stroke-width", "0");
			bar.setAttribute("x", x);
	...
	djd43.charting.Plotters["Default"] = djd43.charting.Plotters.Line;
}
