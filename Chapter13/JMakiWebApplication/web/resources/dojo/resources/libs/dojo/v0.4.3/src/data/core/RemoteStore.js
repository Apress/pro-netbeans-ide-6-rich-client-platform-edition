...
djd43.provide("djd43.data.core.RemoteStore");
djd43.require("djd43.data.core.Read");
djd43.require("djd43.data.core.Write");
...
djd43.experimental("djd43.data.core.RemoteStore");
djd43.lang.declare("djd43.data.core.RemoteStore", [djd43.data.core.Read, djd43.data.core.Write], {_datatypeMap:{}, _jsonRegistry:djd43.json.jsonRegistry, initializer:function (kwArgs) {
	if (!kwArgs) {
		kwArgs = {};
	}
	...
	if (typeof query == "string") {
		...
		return djd43.json.serialize(query);
	}
}, _assertIsItem:function (item) {
	if (!this.isItem(item)) {
		throw new Error("djd43.data.RemoteStore: a function was passed an item argument that was not an item");
	}
	...
	if (attributes) {
		for (var attribute in attributes) {
			...
			if (djd43.lang.isArray(valueOrArrayOfValues)) {
	...
	var oldRegistry = djd43.json.jsonRegistry;
	djd43.json.jsonRegistry = this._jsonRegistry;
	var jsonString = djd43.json.serialize(saveStruct);
	djd43.json.jsonRegistry = oldRegistry;
	requestKw.postContent = jsonString;
}, save:function (keywordArgs) {
	keywordArgs = keywordArgs || {};
	var result = new djd43.Deferred();
	var self = this;
	var bindfunc = function (type, data, evt) {
		if (type == "load") {
			if (result.fired == 1) {
