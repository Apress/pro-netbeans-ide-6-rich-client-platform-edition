...
djd43.provide("djd43.widget.Rounded");
djd43.widget.tags.addParseTreeHandler("dojo:rounded");
...
djd43.deprecated("djd43.widget.Rounded will be removed in version 0.5; you can now apply rounded corners to any block element using djd43.lfx.rounded.", "0.5");
djd43.widget.defineWidget("djd43.widget.Rounded", djd43.widget.ContentPane, {isSafari:djd43.render.html.safari, boxMargin:"50px", radius:14, domNode:"", corners:"TR,TL,BR,BL", antiAlias:true, fillInTemplate:function (args, frag) {
	djd43.widget.Rounded.superclass.fillInTemplate.call(this, args, frag);
	djd43.html.insertCssFile(this.templateCssPath);
	...
	var setCorner = function (currentCorner) {
		...
		if (djd43.lang.inArray(cornersPassed, currentCorner)) {
			this.settings[val] = {radius:this.radius, enabled:true};
		} else {
			this.settings[val] = {radius:0};
		}
	};
	djd43.lang.forEach(cornersAvailable, setCorner, this);
	...
	this.masterCorners = [];
	var boxHeight = djd43.html.getStyle(this.box, "height");
	if (boxHeight == "") {
		boxHeight = "0px";
	}
	var boxWidth = djd43.html.getStyle(this.box, "width");
	var borderWidth = djd43.html.getStyle(this.box, "borderTopWidth");
	if (borderWidth == "") {
		borderWidth = "0px";
	}
	var borderColour = djd43.html.getStyle(this.box, "borderTopColor");
	if (borderWidth > 0) {
		this.antiAlias = true;
	}
	var boxColour = djd43.html.getStyle(this.box, "backgroundColor");
	var backgroundImage = djd43.html.getStyle(this.box, "backgroundImage");
	var boxPosition = djd43.html.getStyle(this.box, "position");
	this.boxHeight = parseInt(((boxHeight != "" && boxHeight != "auto" && boxHeight.indexOf("%") == -1) ? boxHeight.substring(0, boxHeight.indexOf("px")) : this.box.scrollHeight));
	this.boxWidth = parseInt(((boxWidth != "" && boxWidth != "auto" && boxWidth.indexOf("%") == -1) ? boxWidth.substring(0, boxWidth.indexOf("px")) : this.box.scrollWidth));
	this.borderWidth = parseInt(((borderWidth != "" && borderWidth.indexOf("px") !== -1) ? borderWidth.slice(0, borderWidth.indexOf("px")) : 0));
	var test = new djd43.gfx.color.Color(boxColour);
							...
							if (this.antiAlias) {
								for (var inty = (y1 + 1); inty < y2; inty++) {
									if (this.backgroundImage != "") {
										...
										var pixelcolour = djd43.gfx.color.blend(this.boxColour, this.borderColour, this.pixelFraction(intx, inty, borderRadius));
	...
	this.drawPixel = function (intx, inty, colour, transAmount, height, newCorner, image, cornerRadius) {
		...
		if (transAmount != 100) {
			djd43.html.setOpacity(pixel, transAmount);
		}
