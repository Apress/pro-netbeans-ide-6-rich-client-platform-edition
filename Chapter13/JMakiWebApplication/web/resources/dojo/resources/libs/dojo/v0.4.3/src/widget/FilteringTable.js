...
djd43.provide("djd43.widget.FilteringTable");
...
djd43.widget.defineWidget("djd43.widget.FilteringTable", djd43.widget.HtmlWidget, function () {
	...
	return this.store.getByKey(djd43.html.getAttribute(row, "value"));
}, getDataByRow:function (row) {
	return this.store.getDataByKey(djd43.html.getAttribute(row, "value"));
}, getRow:function (obj) {
	...
	var arr = this.store.getFromHtml(this.columns, body, function (obj, row) {
		if (typeof (obj[self.valueField]) == "undefined" || obj[self.valueField] == null) {
			obj[self.valueField] = djd43.html.getAttribute(row, "value");
		}
	...
	function createSortFunction(fieldIndex, dir) {
		...
		return function (rowA, rowB) {
			if (djd43.html.hasAttribute(rowA, "emptyRow")) {
				return 1;
			}
			if (djd43.html.hasAttribute(rowB, "emptyRow")) {
				return -1;
			}
	...
	if (meta.sortType == "__markup__") {
		...
		if (meta.getType() == Date) {
			...
			if (!isNaN(val)) {
				...
				cell.innerHTML = djd43.date.strftime(val, format);
			} else {
				cell.innerHTML = val;
			}
		} else {
	...
	if (this.domNode) {
		...
		djd43.event.connect(body, "onclick", this, "onSelect");
		this.parseData(body);
	}
}});
