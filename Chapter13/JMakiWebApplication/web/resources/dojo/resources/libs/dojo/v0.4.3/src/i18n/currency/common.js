...
djd43.provide("djd43.i18n.currency.common");
djd43.require("djd43.experimental");
djd43.experimental("djd43.i18n.currency");
djd43.require("djd43.regexp");
djd43.require("djd43.i18n.common");
djd43.require("djd43.i18n.number");
djd43.require("djd43.lang.common");
djd43.i18n.currency.format = function (value, iso, flags, locale) {
	flags = (typeof flags == "object") ? flags : {};
	var formatData = djd43.i18n.currency._mapToLocalizedFormatData(djd43.i18n.currency.FORMAT_TABLE, iso, locale);
	...
	flags.signed = false;
	var result = djd43.i18n.number.format(value, flags, locale);
...
djd43.i18n.currency.parse = function (value, iso, locale, flags) {
	if (typeof flags.validate == "undefined") {
		flags.validate = true;
	}
	if (flags.validate && !djd43.i18n.number.isCurrency(value, iso, locale, flags)) {
		return Number.NaN;
	}
	...
	var number = djd43.i18n.number.parse(abs, locale, flags);
	if (sign) {
		number = number * -1;
	}
	return number;
};
djd43.i18n.currency.isCurrency = function (value, iso, locale, flags) {
	flags = (typeof flags == "object") ? flags : {};
	var numberFormatData = djd43.i18n.number._mapToLocalizedFormatData(djd43.i18n.number.FORMAT_TABLE, locale);
	if (typeof flags.separator == "undefined") {
		flags.separator = numberFormatData[0];
	} else {
		if (djd43.lang.isArray(flags.separator) && flags.separator.length == 0) {
			flags.separator = [numberFormatData[0], ""];
		}
	}
	...
	if (typeof flags.symbol == "undefined") {
		...
		if (djd43.lang.isArray(flags.symbol) && flags.symbol.length == 0) {
			flags.symbol = [formatData.symbol, ""];
		}
	}
...
djd43.i18n.currency._mapToLocalizedFormatData = function (table, iso, locale) {
	var formatData = djd43.i18n.currency.FORMAT_TABLE[iso];
	if (!djd43.lang.isArray(formatData)) {
		return formatData;
	}
	return djd43.i18n.number._mapToLocalizedFormatData(formatData[0], locale);
};
(function () {
