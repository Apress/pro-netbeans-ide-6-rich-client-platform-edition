...
djd43.provide("djd43.widget.TreeBasicControllerV3");
...
djd43.require("djd43.widget.TreeV3");
djd43.widget.defineWidget("djd43.widget.TreeBasicControllerV3", [djd43.widget.HtmlWidget, djd43.widget.TreeCommon], function () {
	this.listenedTrees = {};
	...
	if (args.editor) {
		this.editor = djd43.widget.byId(args.editor);
	...
	if (lastFocused && lastFocused.labelNode) {
		...
		djd43.event.disconnect(labelNode, "onblur", this, "onBlurNode");
		labelNode.setAttribute("tabIndex", "-1");
		djd43.html.removeClass(labelNode, "TreeLabelFocused");
	}
	labelNode = node.labelNode;
	labelNode.setAttribute("tabIndex", "0");
	node.tree.lastFocused = node;
	djd43.html.addClass(labelNode, "TreeLabelFocused");
	djd43.event.connectOnce(labelNode, "onblur", this, "onBlurNode");
	djd43.event.connectOnce(labelNode, "onfocus", this, "onFocusNode");
	labelNode.focus();
}, onKey:function (e) {
	...
	djd43.event.browser.addListener(tree.domNode, "onKey", djd43.lang.hitch(this, this.onKey));
	djd43.event.browser.addListener(tree.domNode, "onmousedown", djd43.lang.hitch(this, this.onTreeMouseDown));
	djd43.event.browser.addListener(tree.domNode, "onclick", djd43.lang.hitch(this, this.onTreeClick));
	djd43.event.browser.addListener(tree.domNode, "onfocus", djd43.lang.hitch(this, this.onFocusTree));
	...
	var filter = function (elem) {
		return (elem instanceof djd43.widget.Widget) && elem.isFolder && elem.isExpanded;
	};
	if (nodeOrTree.isTreeNode) {
		this.processDescendants(nodeOrTree, filter, this.collapse);
	} else {
		if (nodeOrTree.isTree) {
			djd43.lang.forEach(nodeOrTree.children, function (c) {
	...
	djd43.lang.forEach(s, function (n) {
		n.expand();
	});
}, expandToLevel:function (nodeOrTree, level) {
	djd43.require("djd43.widget.TreeTimeoutIterator");
	var _this = this;
	...
	var iterator = new djd43.widget.TreeTimeoutIterator(nodeOrTree, callFunc, this);
	...
	if (widgetId) {
		return djd43.widget.byId(widgetId);
	} else {
		if (node == null) {
			return null;
		} else {
			return djd43.widget.manager.byNode(node);
		}
	}
	...
	if (!this.editor) {
		djd43.raise(this.widgetType + ": no editor specified");
	}
	...
	if (save) {
		...
		if (server_data) {
			djd43.lang.mixin(data, server_data);
		}
		if (node.isPhantom) {
			var parent = node.parent;
			var index = node.getParentIndex();
			node.destroy();
			djd43.widget.TreeBasicControllerV3.prototype.doCreateChild.call(this, parent, index, data);
		} else {
	...
	return djd43.widget.TreeBasicControllerV3.prototype.doCreateChild.call(this, parent, index, data);
...
djd43.lang.extend(djd43.widget.TreeBasicControllerV3, {createAndEdit:function (parent, index) {
	var data = {title:parent.tree.defaultChildTitle};
...
djd43.lang.extend(djd43.widget.TreeBasicControllerV3, {canClone:function (child, newParent, index, deep) {
	return true;
...
djd43.lang.extend(djd43.widget.TreeBasicControllerV3, {canDetach:function (child) {
...
djd43.lang.extend(djd43.widget.TreeBasicControllerV3, {canDestroyChild:function (child) {
...
djd43.lang.extend(djd43.widget.TreeBasicControllerV3, {canMoveNotANode:function (child, parent) {
...
djd43.lang.extend(djd43.widget.TreeBasicControllerV3, {canCreateChild:function (parent, index, data) {
