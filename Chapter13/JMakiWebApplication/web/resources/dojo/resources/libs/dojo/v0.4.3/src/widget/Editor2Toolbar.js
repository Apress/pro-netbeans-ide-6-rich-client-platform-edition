...
djd43.provide("djd43.widget.Editor2Toolbar");
...
djd43.lang.declare("djd43.widget.HandlerManager", null, function () {
	...
	djd43.debug("HandlerManager handler " + func + " is not registered, can not remove.");
}, destroy:function () {
	for (var i = 0; i < this._registeredHandlers.length; i++) {
		delete this._registeredHandlers[i];
	}
}});
djd43.widget.Editor2ToolbarItemManager = new djd43.widget.HandlerManager;
djd43.lang.mixin(djd43.widget.Editor2ToolbarItemManager, {getToolbarItem:function (name) {
	var item;
	...
	switch (name) {
	  ...
	  case "forecolor":
	  case "hilitecolor":
		item = new djd43.widget.Editor2ToolbarColorPaletteButton(name);
		break;
	  case "plainformatblock":
		item = new djd43.widget.Editor2ToolbarFormatBlockPlainSelect("formatblock");
		break;
	  case "formatblock":
		item = new djd43.widget.Editor2ToolbarFormatBlockSelect("formatblock");
		break;
	  case "fontsize":
		item = new djd43.widget.Editor2ToolbarFontSizeSelect("fontsize");
		break;
	  case "fontname":
		item = new djd43.widget.Editor2ToolbarFontNameSelect("fontname");
		break;
	  case "inserttable":
	  case "insertcell":
	  case "insertcol":
	  ...
	  case "mergecells":
	  case "splitcell":
		djd43.debug(name + " is implemented in djd43.widget.Editor2Plugin.TableOperation, please require it first.");
		break;
	  case "inserthtml":
	  case "blockdirltr":
	  case "blockdirrtl":
	  ...
	  case "inlinedirltr":
	  case "inlinedirrtl":
		djd43.debug("Not yet implemented toolbar item: " + name);
		break;
	  default:
		djd43.debug("djd43.widget.Editor2ToolbarItemManager.getToolbarItem: Unknown toolbar item: " + name);
	}
	return item;
}});
djd43.addOnUnload(djd43.widget.Editor2ToolbarItemManager, "destroy");
djd43.declare("djd43.widget.Editor2ToolbarButton", null, function (name) {
	this._name = name;
	...
	djd43.event.connect(this._domNode, "onclick", this, "onClick");
	if (!nohover) {
		djd43.event.connect(this._domNode, "onmouseover", this, "onMouseOver");
		djd43.event.connect(this._domNode, "onmouseout", this, "onMouseOut");
	}
}, disableSelection:function (rootnode) {
	djd43.html.disableSelection(rootnode);
	var nodes = rootnode.all || rootnode.getElementsByTagName("*");
	for (var x = 0; x < nodes.length; x++) {
		djd43.html.disableSelection(nodes[x]);
	}
}, onMouseOver:function () {
	var curInst = djd43.widget.Editor2Manager.getCurrentInstance();
	if (curInst) {
		var _command = curInst.getCommand(this._name);
		if (_command && _command.getState() != djd43.widget.Editor2Manager.commandState.Disabled) {
	...
	var curInst = djd43.widget.Editor2Manager.getCurrentInstance();
	var em = djd43.widget.Editor2Manager;
	if (curInst) {
		var _command = curInst.getCommand(this._name);
		if (_command) {
	...
	djd43.html.addClass(this._domNode, this._parentToolbar.ToolbarLatchedItemStyle);
}, enableToolbarItem:function () {
	this._domNode.disabled = false;
	this.removeToolbarItemStyle(this._domNode);
	djd43.html.addClass(this._domNode, this._parentToolbar.ToolbarEnabledItemStyle);
}, disableToolbarItem:function () {
	this._domNode.disabled = true;
	this.removeToolbarItemStyle(this._domNode);
	djd43.html.addClass(this._domNode, this._parentToolbar.ToolbarDisabledItemStyle);
}, highlightToolbarItem:function () {
	djd43.html.addClass(this._domNode, this._parentToolbar.ToolbarHighlightedItemStyle);
}, unhighlightToolbarItem:function () {
	djd43.html.removeClass(this._domNode, this._parentToolbar.ToolbarHighlightedItemStyle);
}, removeToolbarItemStyle:function () {
	djd43.html.removeClass(this._domNode, this._parentToolbar.ToolbarEnabledItemStyle);
	djd43.html.removeClass(this._domNode, this._parentToolbar.ToolbarLatchedItemStyle);
	djd43.html.removeClass(this._domNode, this._parentToolbar.ToolbarDisabledItemStyle);
	this.unhighlightToolbarItem();
}});
djd43.declare("djd43.widget.Editor2ToolbarDropDownButton", djd43.widget.Editor2ToolbarButton, {onClick:function () {
	if (this._domNode && !this._domNode.disabled && this._parentToolbar.checkAvailability()) {
		if (!this._dropdown) {
			this._dropdown = djd43.widget.createWidget("PopupContainer", {});
	...
	djd43.widget.Editor2ToolbarDropDownButton.superclass.destroy.call(this);
}, onDropDownShown:function () {
}, onDropDownDestroy:function () {
}});
djd43.declare("djd43.widget.Editor2ToolbarColorPaletteButton", djd43.widget.Editor2ToolbarDropDownButton, {onDropDownShown:function () {
	if (!this._colorpalette) {
		this._colorpalette = djd43.widget.createWidget("ColorPalette", {});
		...
		djd43.event.connect(this._colorpalette, "onColorSelect", this, "setColor");
		djd43.event.connect(this._dropdown, "open", this, "latchToolbarItem");
		djd43.event.connect(this._dropdown, "close", this, "enableToolbarItem");
	}
...
djd43.declare("djd43.widget.Editor2ToolbarFormatBlockPlainSelect", djd43.widget.Editor2ToolbarButton, {create:function (node, toolbar) {
	this._domNode = node;
	this._parentToolbar = toolbar;
	this._domNode = node;
	this.disableSelection(this._domNode);
	djd43.event.connect(this._domNode, "onchange", this, "onChange");
}, destroy:function () {
	this._domNode = null;
}, onChange:function () {
	...
	if (this._domNode) {
		djd43.widget.Editor2ToolbarFormatBlockPlainSelect.superclass.refreshState.call(this);
		var curInst = djd43.widget.Editor2Manager.getCurrentInstance();
		if (curInst) {
			var _command = curInst.getCommand(this._name);
			if (_command) {
				...
				djd43.lang.forEach(this._domNode.options, function (item) {
...
djd43.declare("djd43.widget.Editor2ToolbarComboItem", djd43.widget.Editor2ToolbarDropDownButton, {href:null, create:function (node, toolbar) {
	djd43.widget.Editor2ToolbarComboItem.superclass.create.apply(this, arguments);
	if (!this._contentPane) {
		djd43.require("djd43.widget.ContentPane");
		this._contentPane = djd43.widget.createWidget("ContentPane", {preload:"true"});
		this._contentPane.addOnLoad(this, "setup");
		this._contentPane.setUrl(this.href);
	}
}, onMouseOver:function (e) {
	if (this._lastState != djd43.widget.Editor2Manager.commandState.Disabled) {
		djd43.html.addClass(e.currentTarget, this._parentToolbar.ToolbarHighlightedSelectStyle);
	}
}, onMouseOut:function (e) {
	djd43.html.removeClass(e.currentTarget, this._parentToolbar.ToolbarHighlightedSelectStyle);
}, onDropDownShown:function () {
	...
	djd43.html.addClass(e.currentTarget, this._parentToolbar.ToolbarHighlightedSelectItemStyle);
}, onMouseOutItem:function (e) {
	djd43.html.removeClass(e.currentTarget, this._parentToolbar.ToolbarHighlightedSelectItemStyle);
}});
djd43.declare("djd43.widget.Editor2ToolbarFormatBlockSelect", djd43.widget.Editor2ToolbarComboItem, {href:djd43.uri.moduleUri("djd43.widget", "templates/Editor2/EditorToolbar_FormatBlock.html"), setup:function () {
	djd43.widget.Editor2ToolbarFormatBlockSelect.superclass.setup.call(this);
	...
	for (var name in this._blockNames) {
		djd43.event.connect(this._blockNames[name], "onclick", this, "onChange");
		djd43.event.connect(this._blockNames[name], "onmouseover", this, "onMouseOverItem");
		djd43.event.connect(this._blockNames[name], "onmouseout", this, "onMouseOutItem");
	}
}, onDropDownDestroy:function () {
	if (this._blockNames) {
		for (var name in this._blockNames) {
	...
	djd43.widget.Editor2ToolbarFormatBlockSelect.superclass.refreshState.call(this);
	if (this._lastState != djd43.widget.Editor2Manager.commandState.Disabled) {
		var curInst = djd43.widget.Editor2Manager.getCurrentInstance();
		if (curInst) {
			var _command = curInst.getCommand(this._name);
...
djd43.declare("djd43.widget.Editor2ToolbarFontSizeSelect", djd43.widget.Editor2ToolbarComboItem, {href:djd43.uri.moduleUri("djd43.widget", "templates/Editor2/EditorToolbar_FontSize.html"), setup:function () {
	djd43.widget.Editor2ToolbarFormatBlockSelect.superclass.setup.call(this);
	var nodes = this._contentPane.domNode.all || this._contentPane.domNode.getElementsByTagName("*");
	this._fontsizes = {};
	...
	for (var name in this._fontsizes) {
		djd43.event.connect(this._fontsizes[name], "onclick", this, "onChange");
		djd43.event.connect(this._fontsizes[name], "onmouseover", this, "onMouseOverItem");
		djd43.event.connect(this._fontsizes[name], "onmouseout", this, "onMouseOutItem");
	}
}, onDropDownDestroy:function () {
	if (this._fontsizes) {
		for (var name in this._fontsizes) {
...
djd43.widget.defineWidget("djd43.widget.Editor2Toolbar", djd43.widget.HtmlWidget, function () {
	...
	djd43.widget.Editor2Toolbar.superclass.destroy.call(this);
}});
