...
djd43.provide("djd43.xml.XslTransform");
djd43.xml.XslTransform = function (xsltUri) {
	djd43.debug("XslTransform is supported by Internet Explorer and Mozilla, with limited support in Opera 9 (no document function support).");
	var IS_IE = djd43.render.html.ie;
	...
	function getActiveXImpl(activeXArray) {
		...
		djd43.raise("Could not find an ActiveX implementation in:\n\n " + activeXArray);
	}
	if (xsltUri == null || xsltUri == undefined) {
		djd43.raise("You must pass the URI String for the XSL file to be used!");
		return false;
	}
	...
	function getIeResultDom(xmlDoc) {
		...
		if (outDoc.parseError.errorCode != 0) {
			...
			djd43.raise("err.errorCode: " + err.errorCode + "\n\nerr.reason: " + err.reason + "\n\nerr.url: " + err.url + "\n\nerr.srcText: " + err.srcText);
		}
		return outDoc;
	}
