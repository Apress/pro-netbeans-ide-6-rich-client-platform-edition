...
djd43.provide("djd43.widget.Form");
djd43.require("djd43.widget.*");
djd43.require("djd43.widget.HtmlWidget");
djd43.widget.defineWidget("djd43.widget.Form", djd43.widget.HtmlWidget, {isContainer:true, templateString:"<form dojoAttachPoint='containerNode' dojoAttachEvent='onSubmit:onSubmit'></form>", formElements:[], ignoreNullValues:false, postCreate:function (args, frag) {
	for (var key in args) {
		if (key == "dojotype") {
			continue;
		}
	...
	for (var i = 0, len = this.formElements.length; i < len; i++) {
		...
		switch (type) {
		  ...
		  case "text":
			...
			break;
		  default:
			djd43.debug("Not supported type (" + type + ")");
			break;
		}
	}
