...
djd43.provide("djd43.lfx.rounded");
...
djd43.lfx.rounded = function (settings) {
	...
	if (typeof (arguments[1]) == "string") {
		nodes = djd43.html.getElementsByClass(arguments[1]);
	} else {
		if (djd43.lang.isArrayLike(arguments[1])) {
			nodes = arguments[1];
			for (var i = 0; i < nodes.length; i++) {
				nodes[i] = djd43.byId(nodes[i]);
			}
		}
	}
	if (nodes.length == 0) {
	...
	for (var i = 0; i < nodes.length; i++) {
		djd43.lfx.rounded.applyCorners(options, nodes[i]);
	}
};
djd43.lfx.rounded.applyCorners = function (options, node) {
	var top = null;
	var bottom = null;
	var contentNode = null;
	var fns = djd43.lfx.rounded._fns;
	var width = node.offsetWidth;
	var height = node.offsetHeight;
	var borderWidth = parseInt(djd43.html.getComputedStyle(node, "border-top-width"));
	var borderColor = djd43.html.getComputedStyle(node, "border-top-color");
	var color = djd43.html.getComputedStyle(node, "background-color");
	var bgImage = djd43.html.getComputedStyle(node, "background-image");
	var position = djd43.html.getComputedStyle(node, "position");
	var padding = parseInt(djd43.html.getComputedStyle(node, "padding-top"));
	...
	if (!djd43.html.isPositionAbsolute(node)) {
		node.style.position = "relative";
	}
	node.style.padding = "0px";
	if (djd43.render.html.ie && width == "auto" && height == "auto") {
		...
		if (options.radii[cc] == 0) {
			...
			if (cc != "br") {
				for (var t = 0, k = corner.childNodes.length; t < k; t++) {
					...
					var barTop = parseInt(djd43.html.getComputedStyle(bar, "top"));
					var barLeft = parseInt(djd43.html.getComputedStyle(bar, "left"));
					var barHeight = parseInt(djd43.html.getComputedStyle(bar, "height"));
					if (cc.charAt(1) == "l") {
...
var count = 0;
djd43.lfx.rounded._fns = {blend:function (clr1, clr2, frac) {
	...
	if (opac != 100) {
		djd43.html.setOpacity(px, (opac / 100));
	}
	px.style.top = y + "px";
	px.style.left = x + "px";
