...
djd43.provide("djd43.widget.svg.Chart");
djd43.require("djd43.widget.HtmlWidget");
djd43.require("djd43.widget.Chart");
...
djd43.widget.defineWidget("djd43.widget.svg.Chart", [djd43.widget.HtmlWidget, djd43.widget.Chart], function () {
	...
	this.vectorNode = document.createElementNS(djd43.svg.xmlns.svg, "svg");
	...
	this.plotArea = document.createElementNS(djd43.svg.xmlns.svg, "g");
	...
	this.dataGroup = document.createElementNS(djd43.svg.xmlns.svg, "g");
	...
	this.axisGroup = document.createElementNS(djd43.svg.xmlns.svg, "g");
	this.plotArea.appendChild(this.axisGroup);
	var stroke = 1;
	var line = document.createElementNS(djd43.svg.xmlns.svg, "line");
	var y = djd43.widget.svg.Chart.Plotter.getY(this.properties.axes.x.plotAt, this);
	...
	var textSize = 10;
	var text = document.createElementNS(djd43.svg.xmlns.svg, "text");
	...
	text.appendChild(document.createTextNode(djd43.math.round(parseFloat(this.properties.axes.x.range.min), 2)));
	this.axisGroup.appendChild(text);
	var text = document.createElementNS(djd43.svg.xmlns.svg, "text");
	text.setAttribute("x", this.properties.width - this.properties.padding.right - (textSize / 2));
	text.setAttribute("y", this.properties.height - this.properties.padding.bottom + textSize + 2);
	text.setAttribute("style", "text-anchor:middle;font-size:" + textSize + "px;fill:#000;");
	text.appendChild(document.createTextNode(djd43.math.round(parseFloat(this.properties.axes.x.range.max), 2)));
	this.axisGroup.appendChild(text);
	var line = document.createElementNS(djd43.svg.xmlns.svg, "line");
	var x = djd43.widget.svg.Chart.Plotter.getX(this.properties.axes.y.plotAt, this);
	...
	text.appendChild(document.createTextNode(djd43.math.round(parseFloat(this.properties.axes.y.range.min), 2)));
	this.axisGroup.appendChild(text);
	var text = document.createElementNS(djd43.svg.xmlns.svg, "text");
	text.setAttribute("x", this.properties.padding.left - 4);
	text.setAttribute("y", this.properties.padding.top + (textSize / 2));
	text.setAttribute("style", "text-anchor:end;font-size:" + textSize + "px;fill:#000;");
	text.appendChild(document.createTextNode(djd43.math.round(parseFloat(this.properties.axes.y.range.max), 2)));
	this.axisGroup.appendChild(text);
	djd43.svg.g.resume();
}, init:function () {
	if (!this.properties.width || !this.properties.height) {
		var box = djd43.html.getContentBox(this.domNode);
	...
	for (var i = 0; i < this.series.length; i++) {
		djd43.widget.svg.Chart.Plotter.plot(this.series[i], this);
	}
	djd43.svg.g.resume();
}, postCreate:function () {
...
djd43.widget.svg.Chart.Plotter = new function () {
	var self = this;
	var plotters = {};
	var types = djd43.widget.Chart.PlotTypes;
	...
	plotters["bar"] = function (series, chart) {
		...
		for (var i = 0; i < series.values.length; i++) {
			...
			var bar = document.createElementNS(djd43.svg.xmlns.svg, "rect");
	...
	plotters["line"] = function (series, chart) {
		var tension = 1.5;
		var line = document.createElementNS(djd43.svg.xmlns.svg, "path");
	...
	plotters["area"] = function (series, chart) {
		...
		for (var i = 0; i < series.values.length; i++) {
			var x = self.getX(series.values[i].x, chart);
			var y = self.getY(series.values[i].value, chart);
			var point = document.createElementNS(djd43.svg.xmlns.svg, "path");
	...
	plotters["bubble"] = function (series, chart) {
		...
		for (var i = 0; i < series.values.length; i++) {
			...
			var point = document.createElementNS(djd43.svg.xmlns.svg, "circle");
			point.setAttribute("stroke-width", 0);
			point.setAttribute("fill", series.color);
