...
djd43.provide("djd43.io.ScriptSrcIO");
djd43.require("djd43.io.BrowserIO");
djd43.require("djd43.undo.browser");
djd43.io.ScriptSrcTransport = new function () {
	...
	this.startWatchingInFlight = function () {
		if (!this.inFlightTimer) {
			this.inFlightTimer = setInterval("djd43.io.ScriptSrcTransport.watchInFlight();", 100);
		}
	};
	...
	this.canHandle = function (kwArgs) {
		return djd43.lang.inArray(["text/javascript", "text/json", "application/json"], (kwArgs["mimetype"].toLowerCase())) && (kwArgs["method"].toLowerCase() == "get") && !(kwArgs["formNode"] && djd43.io.formHasFile(kwArgs["formNode"])) && (!kwArgs["sync"] || kwArgs["sync"] == false) && !kwArgs["file"] && !kwArgs["multipart"];
	};
	...
	this.bind = function (kwArgs) {
		...
		if (kwArgs.sendTransport || jsonpName) {
			...
			if (kwArgs.sendTransport) {
				content["djd43.transport"] = "scriptsrc";
			}
			if (jsonpName) {
				content[jsonpName] = "djd43.io.ScriptSrcTransport._state." + id + ".jsonpCall";
			}
		}
		if (kwArgs.postContent) {
			...
			if (content) {
				query += ((query) ? "&" : "") + djd43.io.argsFromMap(content, kwArgs.encoding, jsonpName);
			}
		}
		if (kwArgs["apiId"]) {
		...
		if (content && content[jsonpName]) {
			...
			state.jsonpCall = function (data) {
				if (data["Error"] || data["error"]) {
					if (djd43["json"] && dojo["json"]["serialize"]) {
						djd43.debug(djd43.json.serialize(data));
					}
					djd43.io.ScriptSrcTransport._finish(this, "error", data);
				} else {
					djd43.io.ScriptSrcTransport._finish(this, "load", data);
				}
			};
		}
		...
		if (kwArgs["preventCache"] || (this.preventCache == true && kwArgs["preventCache"] != false)) {
			state.nocacheParam = "djd43.preventCache=" + new Date().valueOf();
		} else {
			state.nocacheParam = "";
		}
	...
	djd43.io.transports.addTransport("ScriptSrcTransport");
};
window.onscriptload = function (event) {
	var state = null;
	var transport = djd43.io.ScriptSrcTransport;
	...
	switch (event.status) {
	  case djd43.io.ScriptSrcTransport.DsrStatusCodes.Continue:
		callbackName = "partOk";
		break;
	  case djd43.io.ScriptSrcTransport.DsrStatusCodes.Ok:
		callbackName = "load";
		break;
	}
