...
djd43.declare("djd43.widget.Chart", null, function () {
	...
	for (var i = 0; i < this.series.length; i++) {
		var c = djd43.gfx.color.hsl2rgb(hue, sat, lum);
		if (!this.series[i].color) {
			this.series[i].color = djd43.gfx.color.rgb2hex(c[0], c[1], c[2]);
		}
		hue += steps;
	}
	...
	if (!(thead && tbody)) {
		djd43.raise("djd43.widget.Chart: supplied table must define a head and a body.");
	}
	...
	for (var i = 1; i < columns.length; i++) {
		...
		var ds = new djd43.widget.Chart.DataSeries(key, label, plotType, color);
		this.series.push(ds);
	}
	var rows = tbody.rows;
...
djd43.declare("djd43.widget.Chart.DataSeries", null, function (key, label, plotType, color) {
	this.id = "DataSeries" + djd43.widget.Chart.DataSeries.count++;
	this.key = key;
	...
	if (v.x == null || v.value == null) {
		djd43.raise("djd43.widget.Chart.DataSeries.add: v must have both an 'x' and 'value' property.");
	}
	this.values.push(v);
}, clear:function () {
	this.values = [];
...
djd43.requireIf(djd43.render.svg.capable, "djd43.widget.svg.Chart");
djd43.requireIf(!djd43.render.svg.capable && djd43.render.vml.capable, "djd43.widget.vml.Chart");
